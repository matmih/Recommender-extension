...
public class SparseMatrix_d implements IMatrix {

	static final long serialVersionUID=3453435;
	
	protected List<com.rapidminer.improved.HashMap<Integer, Double>> row_list = new ArrayList<com.rapidminer.improved.HashMap<Integer, Double>>();
	int num_cols;
	...
	public com.rapidminer.improved.HashMap<Integer, Double> Get (int x)
			{

		            if (x >= row_list.size())
		                return new com.rapidminer.improved.HashMap<Integer, Double>();
			...
			public double getLocation(int x, int y)
			{
			...
			public void setLocation(int x, int y, double value){
					if (x >= row_list.size())
						for (int i = row_list.size(); i <= x; i++)
							row_list.add( new com.rapidminer.improved.HashMap<Integer, Double>() );
	...
	public SparseMatrix_d(int num_rows, int _num_cols)
	{
	...
	///
	public IMatrix CreateMatrix(int num_rows, int num_columns)
	{
		return new SparseMatrix_d(num_rows, num_columns);
	}

	///
	
	public ArrayList<Tupel<Integer, com.rapidminer.improved.HashMap<Integer, Double>>> NonEmptyRows()
	{
		
		ArrayList<Tupel<Integer,com.rapidminer.improved.HashMap<Integer,Double>>> return_list = new ArrayList<Tupel<Integer, com.rapidminer.improved.HashMap<Integer, Double>>>();

		for (int i = 0; i < row_list.size(); i++)
			if (row_list.get(i).size() > 0){
				return_list.add(new Tupel<Integer,com.rapidminer.improved.HashMap<Integer,Double>>(i,row_list.get(i)));
			}
		return return_list;
	}
	...
	public List<Tupel<Integer, Integer>> NonEmptyEntryIDs()
	{
		...
		ArrayList<Tupel<Integer,com.rapidminer.improved.HashMap<Integer,Double>>> temp=this.NonEmptyRows();
	...
	public IMatrix Transpose()
	{
		...
		SparseMatrix_d transpose = new SparseMatrix_d(NumberOfColumns(), NumberOfRows());
