...
public class Matrix_i implements IMatrix_i {
   ...
   /// <summary>Data array: data is stored in columns.</summary>
		...
		public int dim2;
              ...
              public void setLocation(int i, int j, int value){
		...
		public Matrix_i(int dim1, int dim2)
		{
			if (dim1 < 0)
		...
		public Matrix_i(List<List<Integer>> _data)
		{
		...
		///
		public Matrix_i CreateMatrix(int num_rows, int num_columns)
		{
			return new Matrix_i(num_rows, num_columns);
		}

		///
		public Matrix_i Transpose()
		{
			Matrix_i transpose = new Matrix_i(NumberOfColumns(), NumberOfRows());
			for (int i = 0; i < dim1; i++)
				for (int j = 0; j < dim2; j++)
		...
		public void AddRows(int num_rows)
		{
			if (num_rows > dim1)
			{
				...
				int[] data_new =  new int[num_rows * dim2];
		...
		public void Grow(int num_rows, int num_cols)
		{
			...
			{
				// create new data structure
				int[] new_data =new int[num_rows * num_cols];
				for (int i = 0; i < dim1; i++)
					for (int j = 0; j < dim2; j++)
		...
		/// <param name="i">the row ID</param>
		public void SetRowToOneValue(int i, int v)
		{
			for (int j = 0; j < dim2; j++)
                       setLocation(i,j,v);
		}
		...
		/// <param name="v">the value to be used</param>
		/// <param name="j">the column ID</param>
		public void SetColumnToOneValue(int j, int v)
		{
			for (int i = 0; i < dim1; i++)
                           setLocation(i,j,v);
		}
